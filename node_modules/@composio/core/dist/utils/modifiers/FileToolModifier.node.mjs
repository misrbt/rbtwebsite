import { a as ComposioSensitiveFilePathBlockedError, c as logger, i as ComposioFileUploadPathNotAllowedError, n as ComposioFileUploadAbortedError, r as ComposioFileUploadError, t as ComposioFileNotFoundError } from "../../FileModifierErrors-3fSwIWoa.mjs";
import { b as ComposioRequestCancelledError } from "../../errors-DSIlYfl5.mjs";
import { i as transformProperties, n as schemaHasFileProperty, t as isPlainObject } from "../../FileToolModifier.utils.neutral-B5rT711M.mjs";
import { i as withCancellation, t as dereferenceJsonSchema } from "../../jsonSchema-CFKdONpP.mjs";
import { n as getFileDataAfterUploadingToS3, t as downloadFileFromS3 } from "../../fileUtils.node-BiO1JQsJ.mjs";
...
*/
const resolveFileSchema = (schema) => schema ? dereferenceJsonSchema(schema, { onUnresolved: "sentinel" }) : schema;
const getSchemaVariants = (schema) => [
	...schema?.anyOf ?? [],
	...schema?.oneOf ?? [],
	...schema?.allOf ?? []
];
const jsonSchemaTypeMatchesValue = (schema, value) => {
	const schemaType = schema.type;
	if (Array.isArray(schemaType)) return schemaType.some((type) => jsonSchemaTypeMatchesValue({
		...schema,
		type
	}, value));
...
const findSchemaVariantWithFileProperty = (schema, property, value) => {
	const candidates = getSchemaVariants(schema).filter((variant) => schemaHasFileProperty(variant, property));
	return candidates.find((candidate) => jsonSchemaTypeMatchesValue(candidate, value)) ?? candidates[0];
};
/**
...
const hydrateFiles = async (value, schema, ctx) => {
	if (schema?.file_uploadable) {
		...
		const runBeforeFileUpload = async (path, source) => {
			...
			if (out === false) throw new ComposioFileUploadAbortedError("File upload was aborted because beforeFileUpload returned false.");
			return out;
		};
	...
	if (uploadableVariant) return hydrateFiles(value, uploadableVariant, ctx);
...
const hydrateDownloads = async (value, schema, ctx) => {
	...
	if (getSchemaVariants(schema).length > 0) {
		const downloadableVariant = findSchemaVariantWithFileProperty(schema, "file_downloadable", value);
		if (downloadableVariant) return hydrateDownloads(value, downloadableVariant, ctx);
		return hydrateDownloads(value, void 0, ctx);
	}
...
var FileToolModifier = class {
	...
	async fileUploadModifier(tool, options) {
		...
		try {
			const newArgs = await withCancellation(() => hydrateFiles(args, resolveFileSchema(tool.inputParameters), {
				toolSlug,
				toolkitSlug,
				client: this.client,
			...
			if (error instanceof ComposioRequestCancelledError || error instanceof ComposioSensitiveFilePathBlockedError || error instanceof ComposioFileUploadAbortedError || error instanceof ComposioFileUploadPathNotAllowedError || error instanceof ComposioFileNotFoundError) throw error;
	...
	async fileDownloadModifier(tool, options) {
		...
		const dataWithDownloads = await hydrateDownloads(result.data, resolveFileSchema(tool.outputParameters), {
			toolSlug,
